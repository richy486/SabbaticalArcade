...
func play():
	...
	match game_type:
		...
		2:
			...
			#OS.execute("msedge.exe", ["--kiosk", file_to_run, "--edge-kiosk-type=fullscreen"])
			OS.execute("C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe", ["--kiosk", file_to_run], false) 
